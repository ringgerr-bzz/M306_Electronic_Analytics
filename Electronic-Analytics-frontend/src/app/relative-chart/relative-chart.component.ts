...
export type RelativeChartOptions = {
...
@Component({
  selector: 'app-relative-chart',
  standalone: true,
  imports: [NgApexchartsModule, MatCardModule],
  template: `
    <mat-card>
      <mat-card-title>Relativer Verbrauch & Einspeisung</mat-card-title>
      <mat-card-content>
        <apx-chart
          [series]="opts.series"
          [chart]="opts.chart"
...
export class RelativeChartComponent implements OnChanges {
  @Input() data: { sensorId: string; data: any[] }[] = [];

  public opts: RelativeChartOptions = {
    series: [],
  ...
  ngOnChanges() {
    ...
    const s742 = xs.map(x => {
      ...
      return {x, y: m ? m.relativeValue : 0};
    });
    const s735 = xs.map(x => {
    ...
    this.opts = {
      ...this.opts,
      series: [
        {name: 'Bezug (kWh)', data: s742},
        {name: 'Einspeisung (kWh)', data: s735}
      ],
      xaxis: {
        ...this.opts.xaxis,
        min: xs[0],
